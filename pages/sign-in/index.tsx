      ...
      <div className="flex flex-col justify-center items-center mt-4">
        <div className="w-full">
          <button
            ...
            type="submit"
          >
            <p className="text-slate-300 font-medium">{state.loading ? 'Loading...' : actionText}</p>
          </button>
        </div>
        {!state.forgot && !state.reset && !state.register && (
          <p onClick={() => setState(s => ({ ...s, forgot: true }))} className="text-slate-200 text-lg font-medium">
            Reset Password
          </p>
        )}
        <div className="w-full">
          ...
          >
            ...
            <p className="text-slate-300 font-medium">{state.register ? 'Sign in' : 'Sign up today'}</p>
          </button>
        </div>
      </div>
    </div>
