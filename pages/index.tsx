...
import {
  AddToChatSessionDocument,
  AddToChatSessionSubscription,
...
const Home = () => {
  ...
  const { user, client, claims } = useSession();
  const { register } = useForm({ mode: "all", reValidateMode: "onChange" });

  const url = process.env.NEXT_PUBLIC_HASURA_GRAPHQL_URL_WS!;
              ...
              <div className="flex-col divide-y divide-slate-700">
                {data &&
                  ...
                  data.chat_session.map((session, index) => {
                    const member = session.members.filter(
                      (member) => member.user_id !== claims!["x-hasura-user-id"]
                    );
                    return (
                      <>
                          ...
                          >
                            ...
                            <div className="px-5">
                              <p className="text-slate-400 text-lg font-medium truncate">
                                {member[0].user?.first_name}{" "}
                                {member[0].user?.last_name}
                              </p>
...
function ChatBox({
  ...
  const { data, isLoading } = useGetChatSessionQuery(
    client,
  ...
  const recipient =
    data &&
    data.chatSession?.members.filter(
      (member) => member.user_id !== claims!["x-hasura-user-id"]
    )[0];
  ...
  return (
    ...
    // <div className="border-b p-8 flex w-full items-center justify-between h-24 absolute bg-slate-900 mb-10">
    ...
    //     className="h-full bg-slate-800 flex-1 overflow-hidden"
    //   >
    ...
    //             </div>
    //           </div>
    //           <div className="flex flex-row items-center h-16 rounded-xl border border-slate-600 bg-slate-800 w-full px-4 absolute bottom-0 left-0">
    //             <div>
                      ...
                      <div className="grid grid-cols-12 gap-y-2">
                        {data &&
                          data.chatSession?.messages?.map((message, index) => {
                            ...
                            if (
                              message.from_id === claims!["x-hasura-user-id"]
                            ) {
                              return (
                                <div
                                  key={index}
            ...
            <div className="relative w-full my-6 mx-auto max-w-3xl">
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-slate-800 outline-none focus:outline-none">
                <div className="flex items-start justify-between p-5 border-b border-solid border-slate-600 rounded-t ">
                  <h3 className="text-1xl font=semibold text-slate-300">
                    Select A User
                  </h3>
                  <button
